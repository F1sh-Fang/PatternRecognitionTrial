...
class BalloonDataset(Dataset):
    ...
    def get_cache_path(self,tid):
        return "dataset/cache/" + tid + ".npz"
    def __getitem__(self, idx: int):
        tid: str = list(self.annotations.keys())[idx]
        cache_path = self.get_cache_path(tid)
        ...
        if os.path.exists(cache_path):
            ...
            mask: torch.Tensor = torch.tensor(mask, dtype=torch.float32).unsqueeze(0)  # Single channel mask
...
### 加载数据集
annotations_path = "dataset/balloon/train/via_region_data.json"
dataset_dir = 'dataset/balloon/train'
...
class UNetModel(nn.Module):
    def __init__(self, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS, out_channels=1):
        ...
        # 解码器部分 (Decoder)
        self.decoder4_upconv = self.upconv_block(128, 128)
        self.decoder3_upconv  = self.upconv_block(64, 64)
        self.decoder2_upconv  = self.upconv_block(32, 32)
        self.decoder1_upconv  = self.upconv_block(16, 16)

        self.decoder4 = self.conv3_block(192, 64)
        self.decoder3 = self.conv3_block(96, 32)
        self.decoder2 = self.conv3_block(48, 16)
        self.decoder1 = self.conv3_block(24, 8)
...
if __name__ == '__main__':
    ...
    model = train_model(model, criterion, optimizer, train_loader, test_loader, num_epochs=500)
